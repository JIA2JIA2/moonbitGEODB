...
- **基础类型编码**：大端字节序（Big-Endian）
  - `Int32`: 4 字节
  - `Int64`: 8 字节
  - `Double`: 8 字节（IEEE 754 位模式）
  - `String`: 长度前缀（Int32）+ UTF-8 字节
  - `Bytes`: 长度前缀（Int32）+ 原始字节
- **Double 编码**：使用 `reinterpret_as_int64()`/`reinterpret_as_double()` 确保 IEEE 754 位模式无损转换
- **CRC32 校验**：每个条目块带 CRC32 校验，确保数据完整性

### 数据安全策略

1. **原子写入**：保存时先写入 `.tmp` 临时文件，成功后再重命名为目标文件，避免中途崩溃导致数据损坏
2. **自动备份**：通过 `backup()` 方法创建 `.bak` 备份文件
3. **完整性检查**：`check()` 方法验证文件格式、CRC32 校验、版本号
4. **版本兼容**：支持版本号扩展，便于未来格式升级
