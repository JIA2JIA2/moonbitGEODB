...
| 操作 | 1k | 10k | 扩展趋势 | 实现 |
|------|-----|------|---------|------|
| **构建 DB** | **10,021** | **132,909** | ~O(n) ✓ | bulk_insert + dual-grid |
| 构建/条目 | 10.0 μs | 13.3 μs | 近常数 ✓ | |
| BBox 查询 | **6** | **50** | O(log n) ✓ | adaptive fine/coarse/R-tree |
| KNN (k=5) | **43** | **90** | O(log n) ✓ | R-tree best-first |
| Within 100km | 9 | 53 | 亚线性 ✓ | unsorted bbox → haversine |
| K-Means (k=3) | 4,380 | 105,090 | O(n·iters·k) | K-Means++ 初始化 |
| **DBSCAN (eps=50km)** | 7,372 | 287,124 | O(n·k) 扩张 | clustered[] 去重队列 |
| TSP 2-opt | **49,548** | **1,128,343** | O(n²·rounds)，实测亚线性 | edge 数组 + 0 HashMap |
| **Moran's I KNN** | 27,748 | 653,456 | **O(n·k·log n) ✓** | 真 R-tree KNN + 行标准化 |
| **Kriging (k=12)** | 8,184 | 21,250 | O(n·log n + k²) | R-tree KNN + 变异函数 |
